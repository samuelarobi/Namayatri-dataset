CREATE TABLE  trips(tripid integer,
					faremethod integer,
					fare integer,
					loc_from integer,
					loc_to integer,
...
INSERT INTO trips(tripid,faremethod,
				  fare,loc_from,loc_to,driverid,
				  custid,distance,duration) 
values
...
INSERT INTO trips_details1(tripid,loc_from,searches,searches_got_estimate,searches_for_quotes,
						  searches_got_quotes,customer_not_cancelled, driver_not_cancelled,
						  otp_entered,end_ride)
...
CREATE TABLE  trips_details2(tripid integer,
							 ...
							 otp_entered integer,
							 end_ride integer); 
INSERT INTO trips_details2(tripid,loc_from,searches,searches_got_estimate,
						   searches_for_quotes,searches_got_quotes,customer_not_cancelled,
						   driver_not_cancelled,otp_entered,end_ride) 
values('999','33','1','1','1','1','1','0','0','0'),
...
CREATE TABLE  trips_details3(tripid	integer,
							 ...
							 otp_entered integer,
							 end_ride integer); 
INSERT INTO trips_details3(tripid,loc_from,searches,searches_got_estimate,
						   searches_for_quotes,searches_got_quotes,
				  customer_not_cancelled,driver_not_cancelled,otp_entered,end_ride) 
 values('1625','37','1','1','0','0','0','0','0','0'),
...
CREATE TABLE  trips_details4 (tripid integer,
							  loc_from integer,searches integer,
							  searches_got_estimate integer,
							  searches_for_quotes	integer,
							  searches_got_quotes	integer,
							  customer_not_cancelled integer,
...
-- this is to check if thre are duplicate ID
select tripid, count(tripid) cnt from trips_details1
group by tripid
having count(tripid)>1;

--total drivers THEN RENAME AS TOTAL_DRIVERS
SELECT COUNT(DISTINCT driverid) AS Total_drivers
from trips;
select * from trips;


-- total earnings RENAME AS EARNING 
SELECT SUM(fare) AS Earnings FROM trips;


-- total Completed trips CHECK FRO THE TRIPS_detailsTABLE SINCE THEIR IS end_ride COLUMN
...
--total searches which got quotes(1996)
...
SELECT COUNT( driver_not_cancelled) from trips_details1;
...
-- which driver 2, customer 2 pair had more orders
select*,rank() over ( order by cnt desc) rnk from
...
select *from 
select sum(searches_got_estimate),sum(searches)from trips_details1;
-- estimate to search for quote rates
--same above syntax just change the search_for _quote _rate
...
--which duration got the highest number of trips in the location present 
...
--the area thta got the higesty number of fares
...
limit 1;
select *from trips_details1;
-- location that got the highest number of  drivers cancellations 
...
group by loc_from)
limit 2;
-- the higest customer cancellation in a area
select*, rank() over(order by cus desc) rnk
from
(select loc_from , sum(customer_not_cancelled) cus
from trips_details1
group by loc_from)
limit 2;
...
group by duration)
limit 1
-- to view the amount of fare made just cange the syntax to fare 
select*, rank() over(order by fare desc) rnk
from
